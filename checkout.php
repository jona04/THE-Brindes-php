...
//se usuario estiver logado captura os dados do usuario
if(isset($_SESSION['email_the'])){
	...
	//captura o peso, altura, largura e comprimento dos produtos do usuario logado
	$qr_medidas = mysql_query("SELECT car_peso,car_largura,car_altura,car_comprimento FROM carrinho WHERE car_email = '$email'") or die(mysql_error());
	$num_itens = mysql_num_rows($qr_medidas);
	if($num_itens > 0){
		//somatório dos lados de todos os produtos
		for($i = 0; $pega_medidas = mysql_fetch_assoc($qr_medidas); $i++){
			//altiura e peso total , somatorio
			$peso_total += $pega_medidas['car_peso'];
			$altura_total += $pega_medidas['car_altura'];
			
			//pega altura e comprimento max, no primeiro loop, se nao fizer isso ele vai comparar sempre o mesmo valor
			if($i == 0){
				$largura_max = $pega_medidas['car_largura'];
				$comprimento_max = $pega_medidas['car_comprimento'];
			}
			//aqui pegamemos a maior largura de todos os produtos com o vetor aux e max
			$largura_aux[$i] = $pega_medidas['car_largura'];
			//echo $largura_aux[$i].'<br>';
			//echo  'Largura_aux['.$i.']'.$largura_aux[$i].'<br>';
			//se a largura do produto atual form maior q a anterior, a largura max é atualizada
			if($i > 0 && $largura_aux[$i] > $largura_aux[$i-1])
				$largura_max =  $largura_aux[$i];
	
			//aqui pegamemos a maior compimento de todos os produtos com o vetor aux e max
			$comprimento_aux[$i] = $pega_medidas['car_comprimento'];
			//echo  'Comprimento_aux['.$i.']'.$comprimento_aux[$i].'<br>';
			//se a largura do produto atual form maior q a anterior, a largura max é atualizada
			if($i > 0 && $comprimento_aux[$i] > $comprimento_aux[$i-1])
				$comprimento_max =  $comprimento_aux[$i];		
		}
		
		//calculo da cubagem para frete via PAC
		if($altura_total>105){//se altura ultrapassar o limite maximo em cm
			$calculo_lados = $altura_total * $largura_max * $comprimento_max;
			//altura recebe o valor max permitido
			$altura_total = 105;
			//calculo para achar outro valor qualquer para o comprimento(podia ser a largura tbm) que dê o mesmo resultado do calculo lados
			$comprimento_max = $calculo_lados / ($altura_total * $largura_max);
			//echo "<br>".$comprimento_max."<br>";
			$resultado_cubagem = ($altura_total * $largura_max * $comprimento_max) / 6000;
		}else{
			$resultado_cubagem = ($altura_total * $largura_max * $comprimento_max) / 6000;
		}
		
		/*echo "peso total = ".$peso_total."<br>";
		echo "largura max = ".$largura_max;
		echo "<br>comprimneto max = ".$comprimento_max;
		echo "<br>Alttura total = ". $altura_total;
		echo "<br>Resultados da soma e divisão dos lados = ". $resultado_cubagem;
	*/
		//$altura = 2;
		//$largura = 11;
		//$comprimento = 16;
		...
		if($altura_total < 2){
			...
			$altura = (int)$altura_total;}
		if($comprimento_max < 16){
			$comprimento = 16;}
		else{
			$comprimento = (int)$comprimento_max;}
		if($largura_max < 11) {
			$largura = 11;}
		else{
			$largura = (int)$largura_max;
		}
	}//fim if se existir proutos no carrinho
}//fim if se usuario estiver logado
else{
	//inicializa variaveis
	$peso_total = 0;
	$altura_total = 0;
	
	//$largura_aux = array(10);
	//$comprimento_aux = array(10);
	//verifica todos os produtos pela sessão
	...
	static $i = 0;
		...
		if(substr($nome4,0,13) == 'produtosPers_'){
			...
			//captura o peso, altura, largura e comprimento dos produtos do usuario NAO logado			
			$qr_medidas = mysql_query("SELECT pers_peso,pers_altura,pers_largura,pers_comprimento FROM produto_personalizado WHERE pers_foto_id = '$id4'") or die(mysql_error());

			$pega_medidas = mysql_fetch_assoc($qr_medidas);
			//altura e peso total , somatorio
			$peso_total += $pega_medidas['pers_peso'];
			$altura_total += $pega_medidas['pers_altura'];
			
			//pega altura e comprimento max, no primeiro loop, se nao fizer isso ele vai comparar sempre o mesmo valor
			if($i == 0){
				$comprimento_max = $pega_medidas['pers_comprimento'];
				$largura_max = $pega_medidas['pers_largura'];
			}
			//aqui pegamemos a maior largura de todos os produtos com o vetor aux e max
			$largura_aux[$i] = $pega_medidas['pers_largura'];
			...
			//aqui pegamemos a maior compimento de todos os produtos com o vetor aux e max
			$comprimento_aux[$i] = $pega_medidas['pers_comprimento'];
			//echo  'Comprimento_aux['.$i.']'.$comprimento_aux[$i].'<br>';
			//se a largura do produto atual form maior q a anterior, a largura max é atualizada
			if($i > 0 && $comprimento_aux[$i] > $comprimento_aux[$i-1])
				$comprimento_max =  $comprimento_aux[$i];		
		...
		//contador para saber em qual pronduto estamos
...
if(isset($_GET['usu']) == 'comp'){
	...
	//a referencia receberá 'U' seguido do id do usuario que comprou, mais 'P' seguido do id 
	$paymentRequest->setReference("I9635");
	
	// Informando as credenciais  
$credentials = new PagSeguroAccountCredentials(  
    'jonatas@teresinabrindes.com.br',   
    'D11B06B5F278497EAE280BD40A67E469' 
//$credentials = new PagSeguroAccountCredentials(  
...
<div class="centraliza_checkout" style="width:1020px; margin: 40px auto 0 auto;">
	...
	<table>
		...
		<tr>
			...
			$frete->setAltura($altura/*$row_new_produtos['pro_altura']*/);
			// Informa o comprimento da encomenda
			$frete->setComprimento($comprimento/*$row_new_produtos['pro_comprimento']*/);
			// Informa a largura da encomenda
			$frete->setLargura($largura/*$row_new_produtos['pro_altura']*/);
	  ...
	  <tr>
		  <?php 
			...
			// Informa a altura da encomenda
			$frete2->setAltura($altura/*$row_new_produtos['pro_altura']*/);
			// Informa o comprimento da encomenda
			$frete2->setComprimento($comprimento/*$row_new_produtos['pro_comprimento']*/);
			// Informa a largura da encomenda
			$frete2->setLargura($largura/*$row_new_produtos['pro_altura']*/);
			// Informa o serviço. 41106 = PAC
